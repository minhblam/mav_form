...
void line_form(float x_off, int k){ //Where off is offset dist and k is the number of drones
  for (int i = 0; i<k-1 ;k++){ //For each drone

    //If statement to alternate sides or something NEED HERE
    for (int n = 0;  n<wp_subpose.size()-1; n++){ //For each WP
      wp_pose.header.seq = i;
      wp_pose.pose.position.x = wp_subpose[n].poses.position.x + OFFSET_HERE;
      wp_pose.pose.position.y = wp_subpose[n].poses.position.y + OFFSET_HERE;
      wp_pose.pose.position.z = wp_subpose[n].poses.position.z + OFFSET_HERE;
